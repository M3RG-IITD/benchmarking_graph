...
def main(N=3, dt=1.0e-3, useN=None, withdata=None, datapoints=100, mpass=1, grid=False, stride=100, ifdrag=0, seed=42, rname=0, saveovito=0, trainm=1, runs=10, semilog=1, maxtraj=1, plotthings=False, redo=0):
        ...
        try:
            ...
            if plotthings:
                for key, traj in {"actual": actual_traj, "pred": pred_traj}.items():
                    ...
                    # plt.xlabel("Time step", ax=axs[0])
                    plt.xlabel("Time step")
                    # plt.ylabel("Energy", ax=axs[0])
                    plt.ylabel("Energy")
